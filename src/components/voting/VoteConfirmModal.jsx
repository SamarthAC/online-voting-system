...
function VoteConfirmModal({ candidate, voterEmail, onCancel, onConfirm, isSubmitting }) {
  if (!candidate) {
    return null
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4">
      <div className="glass-card w-full max-w-lg rounded-3xl p-6 fade-up">
        ...
        <p className="mt-2 text-sm text-slate-600">
          You are voting as <span className="font-semibold">{voterEmail}</span>. This action is final
        ...
        <div className="mt-5 rounded-2xl border border-slate-200 bg-white/75 p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <img
              ...
              className="h-24 w-full rounded-xl border border-slate-200 bg-white object-cover"
            />
            <img
              src={candidate.personalSymbol}
              alt={`${candidate.name} personal symbol`}
              className="h-24 w-full rounded-xl border border-slate-200 bg-white object-cover"
            />
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <Button type="button" onClick={onConfirm} disabled={isSubmitting}>
            {isSubmitting ? 'Submitting Vote...' : `Confirm Vote For ${candidate.name}`}
          </Button>
        </div>
      </div>
    </div>
